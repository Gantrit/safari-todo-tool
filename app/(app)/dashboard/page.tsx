...
export default async function DashboardPage() {
  ...
  return (
    <div className="page-shell">
      ...
      {!board && (
        <section className="app-card mb-8 flex flex-col gap-6 p-7 sm:flex-row sm:items-center sm:justify-between lg:p-8">
          <div className="max-w-2xl">
            <p className="page-eyebrow">Getting started</p>
            <h2 className="text-xl font-extrabold tracking-[-.025em]">Set up your workspace</h2>
            <p className="mt-2 text-sm leading-6 muted-text">Create your first workspace to unlock team boards, tasks, templates and approvals.</p>
          </div>
      ...
      <section className="mb-8 grid items-start gap-7 xl:grid-cols-[1.45fr_.55fr]">
        ...
        <article className="app-card overflow-hidden">
          ...
          {notifications?.length ? <div>{notifications.map((notification: any) => <div key={notification.id} className="dashboard-row"><p className="text-[13px] leading-5">{notification.message}</p><p className="mt-2 text-[9.5px] font-bold uppercase tracking-[.1em]" style={{ color: 'var(--muted)' }}>{notification.type.replaceAll('_', ' ')}</p></div>)}</div> : <div className="card-empty"><p className="text-sm" style={{ color: 'var(--muted)' }}>You are all caught up.</p></div>}
          <div className="border-t p-4" style={{ borderColor: 'var(--border)' }}><Link href="/notifications" className="btn btn-secondary w-full">View notifications</Link></div>
        </article>
      </section>
