...
export default function TaskForm({ boardId, memberId, section, members, currentUser, onCreated, onCancel }: TaskFormProps) {
  ...
  const fieldClass = 'min-h-12 w-full rounded-[10px] border px-4 text-sm outline-none transition-colors hover:border-[var(--border-strong)] focus:border-[var(--accent)]'
  const fieldStyle = { background: 'var(--surface2)', borderColor: 'var(--border)', color: 'var(--text)' }
  const labelClass = 'mb-2.5 block text-[10.5px] font-bold uppercase tracking-[.1em]'

  return (
    <form onSubmit={handleSubmit}>
      <div className="grid gap-7 p-5 sm:p-8 lg:grid-cols-[minmax(0,1.3fr)_minmax(290px,.7fr)] lg:gap-8">
        <div className="space-y-6">
          ...
          <div>
            ...
            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={4} className={`${fieldClass} min-h-32 resize-y py-3.5 leading-6`} style={fieldStyle} placeholder="Add context, expected outcome, or handoff notes." />
          </div>
          ...
          <div>
            ...
            <textarea value={checklist} onChange={(e) => setChecklist(e.target.value)} rows={4} className={`${fieldClass} min-h-28 resize-y py-3.5 leading-6`} style={fieldStyle} placeholder={'Collect source files\nSubmit first draft\nFinal QA'} />
            <p className="mt-2 text-[11.5px] leading-5" style={{ color: 'var(--muted)' }}>One checklist item per line.</p>
          </div>
          <div className="grid gap-5 sm:grid-cols-2">
        ...
        <aside className="self-start overflow-hidden rounded-[14px] border" style={{ background: 'var(--surface2)', borderColor: 'var(--border)' }}>
          ...
          <div className="border-t pt-5" style={{ borderColor: 'var(--border)' }}>
            <div className="mb-4 flex items-center justify-between gap-3"><span className="flex items-center gap-2 text-xs font-bold"><Repeat2 size={14} style={{ color: 'var(--text-secondary)' }} /> Recurring task</span><Toggle checked={recurringEnabled} onChange={setRecurringEnabled} compact /></div>
      ...
      {error && <div className="mx-5 mb-5 rounded-[10px] border px-4 py-3.5 text-sm sm:mx-8" style={{ background: 'var(--red-dim)', borderColor: 'rgba(255,98,98,.3)', color: 'var(--red)' }}>{error}</div>}
      <footer className="sticky bottom-0 z-10 flex flex-col-reverse gap-3 border-t px-5 py-5 sm:flex-row sm:items-center sm:justify-end sm:px-8 sm:py-6" style={{ borderColor: 'var(--border)', background: 'var(--surface)' }}>
